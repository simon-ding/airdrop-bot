...
func (aq *AccountQuery) QueryStepRuns() *StepRunQuery {
	query := (&StepRunClient{config: aq.config}).Query()
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withStepRuns: aq.withStepRuns.Clone(),
...
func (aq *AccountQuery) WithStepRuns(opts ...func(*StepRunQuery)) *AccountQuery {
	query := (&StepRunClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withStepRuns = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withStepRuns; query != nil {
		if err := aq.loadStepRuns(ctx, query, nodes,
			func(n *Account) { n.Edges.StepRuns = []*StepRun{} },
			func(n *Account, e *StepRun) { n.Edges.StepRuns = append(n.Edges.StepRuns, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadStepRuns(ctx context.Context, query *StepRunQuery, nodes []*Account, init func(*Account), assign func(*Account, *StepRun)) error {
	...
	query.Where(predicate.StepRun(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.StepRunsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_step_runs
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_step_runs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_step_runs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
