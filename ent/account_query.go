...
func (aq *AccountQuery) QueryStepRuns() *StepRunQuery {
	query := &StepRunQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withStepRuns; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.StepRuns = []*StepRun{}
		}
		query.withFKs = true
		query.Where(predicate.StepRun(func(s *sql.Selector) {
			s.Where(sql.InValues(account.StepRunsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_step_runs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_step_runs" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.StepRuns = append(node.Edges.StepRuns, n)
		}
	}
