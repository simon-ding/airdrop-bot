...
func (sr *StepRun) assignValues(columns []string, values []any) error {
...
func (sr *StepRun) Value(name string) (ent.Value, error) {
...
func (sr *StepRun) QueryAccount() *AccountQuery {
	return NewStepRunClient(sr.config).QueryAccount(sr)
}

// QueryNode queries the "node" edge of the StepRun entity.
func (sr *StepRun) QueryNode() *NodeQuery {
	return NewStepRunClient(sr.config).QueryNode(sr)
}
...
func (sr *StepRun) Update() *StepRunUpdateOne {
	return NewStepRunClient(sr.config).UpdateOne(sr)
}
