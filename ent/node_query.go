...
func (nq *NodeQuery) QueryStepRuns() *StepRunQuery {
	query := (&StepRunClient{config: nq.config}).Query()
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withStepRuns: nq.withStepRuns.Clone(),
...
func (nq *NodeQuery) WithStepRuns(opts ...func(*StepRunQuery)) *NodeQuery {
	query := (&StepRunClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withStepRuns = query
	return nq
}
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := nq.withStepRuns; query != nil {
		if err := nq.loadStepRuns(ctx, query, nodes,
			func(n *Node) { n.Edges.StepRuns = []*StepRun{} },
			func(n *Node, e *StepRun) { n.Edges.StepRuns = append(n.Edges.StepRuns, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadStepRuns(ctx context.Context, query *StepRunQuery, nodes []*Node, init func(*Node), assign func(*Node, *StepRun)) error {
	...
	query.Where(predicate.StepRun(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.StepRunsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.node_step_runs
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_step_runs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "node_step_runs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
