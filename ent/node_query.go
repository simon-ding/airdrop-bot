...
func (nq *NodeQuery) QueryStepRuns() *StepRunQuery {
	query := &StepRunQuery{config: nq.config}
...
func (nq *NodeQuery) sqlAll(ctx context.Context) ([]*Node, error) {
	...
	if query := nq.withStepRuns; query != nil {
		...
		query.Where(predicate.StepRun(func(s *sql.Selector) {
			s.Where(sql.InValues(node.StepRunsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.node_step_runs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "node_step_runs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "node_step_runs" returned %v for node %v`, *fk, n.ID)
			}
